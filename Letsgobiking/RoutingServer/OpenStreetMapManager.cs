...
namespace RoutingServer
{
    class OpenStreetMapManager
    {
        ...
        {
            if (!Producer.estConnecter() && start == "InProcess" && end == "InProcess") {
        ...
        private static async Task<List<string>> ComputeItineraireGlobal(string start, string end) {
            ...
            string startNearestContrat = await CalculStation.findNearestContratAsync(start);
            string endNearestContrat = await CalculStation.findNearestContratAsync(end);
            bool onBike = false;

            //regarde la station la plus proche du départ à pied
            InformationStation startCoordNearestStation = await CalculStation.findNearestStationAsync(start, startNearestContrat, onBike);
            onBike = true;
            //regarde la station la plus proche de l'arrivé à vélo
            InformationStation endCoordNearestStation = await CalculStation.findNearestStationAsync(end, endNearestContrat, onBike);
        ...
        public static async Task<string> convertAddressToPointAsync(string address)
        {
            ...
            JsonElement coordinates = jsonResponseBody.RootElement.GetProperty("features")[0].GetProperty("geometry").GetProperty("coordinates");
        ...
        private static void SendSteps(int nombreMessage)
        {
            ...
            if (numeroEtapeActuel < tailleItineraire)
            {
                ...
                {
                    list.Add("FINI");
                    Producer.envoyerMessage(list.ToArray());
                    ReinitialiseVariable();
                    return;
                }
        ...
        private static async void FindNewItineraryBike(string end) {
            string endNearestContrat = await CalculStation.findNearestContratAsync(end);
            InformationStation endCoordNearestStation = await CalculStation.findNearestStationAsync(end, endNearestContrat, true);
